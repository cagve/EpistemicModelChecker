...
private Predicate q = new Predicate("q");
	...
	public boolean WorldReasoner(RelationalFormula formula, EpistemicModel model, World world){
		...
		if(formula.getClass()==atom.getClass()) {
			...
			if(WorldReasoner(imp1,model,world)==true
					&& WorldReasoner(imp2,model,world)==true) {
				bol=true;
			}else {
				bol=false;
			}
	...
	public String runReasoner(String formula, String modelPath) throws IOException {
		...
		for (int y=0;y<worldSet.getWorldSet().size();y++) {
			result=result+"The formula "+t.convertNormal(formula)+ " is "+ this.WorldReasoner(formulaFinal, model, worldSet.getWorldSet().get(y)) + " in w"+y+"\n";
		}
		result=result+"The formula "+t.convertNormal(formula)+" is "+ this.ModelReasoner(formulaFinal,model)+" in the model";
		return result;
	}
}
